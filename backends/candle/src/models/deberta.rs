...
impl DebertaV2DisentangledSelfAttention {
    ...
    fn forward(
        ...
        let q = chunks[0].contiguous()?.reshape((b * h, l, d))?;
        ...
        let alpha: f32 = 1.0 / ((d * n_terms) as f32).sqrt();
        let alpha_t = Tensor::new(alpha, device)?.to_dtype(q.dtype())?; // scalar tensor

        if self.use_cublaslt {
            #[cfg(feature = "cuda")]
            {
                // fused C = alpha*rel_bias + mask  (contiguous, dtype==Q/K)
                let fused_c_owned: Option<Tensor> = {
                    let rel_scaled = match rel_bias_bhl.as_ref() {
                        Some(rb) => Some(rb.broadcast_mul(&alpha_t)?.to_dtype(q.dtype())?),
                        None => None,
                    };
            ...
            #[cfg(not(feature = "cuda"))]
            {
                unreachable!("use_cublaslt set without CUDA feature");
            }
        }
...
impl DebertaV2Attention {
    ...
    fn forward(
        ...
        c2p_idx_bhl: Option<&Tensor>,
        p2c_idx_bhl: Option<&Tensor>,
    ) -> Result<Tensor> {
        let _enter = self.span.enter();
...
struct DebertaV2Encoder {
    ...
    // Full-size (1, Lmax, Lmax) index maps in U32; slice to (L,L) at runtime.
    c2p_full_u32: Option<Tensor>,
    p2c_full_u32: Option<Tensor>,
    span: tracing::Span,
}
...
impl DebertaV2Encoder {
    pub fn load(vb: VarBuilder, config: &DebertaV2Config) -> Result<Self> {
        ...
        // Precompute full c2p/p2c index maps once (U32) to avoid per-L caches.
        let (c2p_full_u32, p2c_full_u32) = if let Some(rp_full) = &rel_pos_full {
            let att_span = if position_buckets > 0 {
                position_buckets
            } else {
                max_relative_positions
            };
            let att_span_t = Tensor::new(&[att_span as i64], vb.device())?;
            let c2p = rp_full
    ...
    fn get_c2p_p2c_indices(&self, l: usize) -> Result<(Tensor, Tensor)> {
        let c2p = self
            .c2p_full_u32
            .as_ref()
            .expect("c2p_full_u32 missing")
            .narrow(1, 0, l)?
            .narrow(2, 0, l)?
            .squeeze(0)?
            .contiguous()?; // (L,L) U32
...
impl DebertaV2Model {
    pub fn load(vb: VarBuilder, config: &DebertaV2Config, model_type: ModelType) -> Result<Self> {
        ...
        let neg_mask_value =
            Tensor::from_slice(&[neg_mask_value_f32], (1,), vb.device())?.to_dtype(vb.dtype())?;
